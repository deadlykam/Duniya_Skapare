...
@export var _lbl_camera_controls: Label
...
func _process(delta) -> void:
	...
	if _control_1.is_camera_control():
		_lbl_current_mode.text = "Current Mode: Move Camera"
		_lbl_camera_controls.visible = true
		_lbl_tile_controls.visible = false
	else:
		_lbl_current_mode.text = "Current Mode: Tile Checker"
		_lbl_camera_controls.visible = false
